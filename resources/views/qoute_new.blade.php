...
<div class="card">
    ...
    <div class="card-body">
        <form action="" id="frm" method="POST" enctype="multipart/form-data">
            ...
            <div class="card">
				...
				<table class="table-responsive-md table tbl-qoute-items" id="items_table">
                        ...
                        <tbody>
                            @foreach (old('items', ['']) as $index => $itemImages)
                                <tr id="item{{ $index }}" style='display:none'>
                                    <td>
                                        @if (!empty($oldItem))
                                         @php 
                                            $item_image = explode("|", $item_images)[0];
                                         @endphp
                                         <image src="/storage/item_images/{{$image}}" onclick="previewImages({{$item_images}})"> 
                                        @endif
                                    </td>
                                    <td>
                                        <input type="text" name="item_names[]" class="form-control" value="{{ old('item_names.' . $index) ?? '1' }}" readonly required>
                                    </td>
									<td>
                                    ...
                                    <td>
                                        <input type="number" name="quantities[]" class="form-control" value="{{ old('quantities[' . $index . ']') ?? '1' }}" readonly required>
                                    </td>
									<td>
                                        <input type="number" name="prices[]" class="form-control" value="{{ old('prices.' . $index) ?? '1' }}" readonly required>
                                    </td>
                                    <td>
                                        <input type="number" name="moqs[]" class="form-control" value="{{ old('moqs.' . $index) ?? '1' }}" readonly required>
                                    </td>	
                                    <td>
                                        <input type="text" name="notes[]" class="form-control" value="{{ old('notes.' . $index) ?? '1' }}" readonly required>
                                    </td>									
                                </tr>
                            @endforeach
        ...
        <!-- Images Carousel -->
        <div class="modal fade" id="itemImagesModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" dir="rtl">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">صور المنتج</h4>
                    </div>
...
<script>
  ...
  function previewImages(item_images){
        ...
        for(var=key; key<item_images.length; key++)
        {   if (key == 0) 
                    $('#items-image-viewer').append( "<div class='carousel-item active'>");
            else
                    $('#items-image-viewer').append( "<div class='carousel-item'>");

            $('#items-image-viewer').append("<img src='/storage/item_images/" + val.image_name + "'>"); 
